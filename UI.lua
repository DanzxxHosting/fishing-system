...
-- Variabel untuk semua remotes yang ditemukan
local fishingRemotesList = {}
...
local function FindFishingRemotes()
    ...
    -- Reset list
    fishingRemotesList = {}
    fishingRemotes.WaitingForBite = nil
    fishingRemotes.MinigameStarted = nil
    fishingRemotes.ObtainedNewFishNotification = nil
    fishingRemotes.CompleteFishing = nil
    
    -- Fungsi untuk scan remotes
    local function ScanForRemotes(parent, prefix)
        for _, child in pairs(parent:GetDescendants()) do
            if child:IsA("RemoteEvent") or child:IsA("RemoteFunction") then
                local fullName = prefix and (prefix .. "." .. child.Name) or child.Name
                fishingRemotesList[fullName] = child
                fishingRemotesList[child.Name] = child
                
                -- Check for specific remotes
    ...
    if controllers then
        ScanForRemotes(controllers, "_Controllers")
        print("[Fishing] Found remotes in _Controllers!")
    end
    
    -- Cari dari Controllers
    local controllersFolder = ReplicatedStorage:FindFirstChild("Controllers")
    if controllersFolder then
        ScanForRemotes(controllersFolder, "Controllers")
        print("[Fishing] Found remotes in Controllers!")
    end
    
    -- Cari dari Fishing atau Fish folder
    local function FindFishingFolders()
        for _, child in pairs(ReplicatedStorage:GetDescendants()) do
            if child:IsA("Folder") then
                local name = child.Name:lower()
                if name:find("fish") or name:find("fishing") then
                    ScanForRemotes(child, "FishingFolder")
                end
            end
        end
    end
    ...
    -- Scan seluruh ReplicatedStorage sebagai fallback
    ...
    -- Print total remotes found
    local totalRemotes = 0
    for _ in pairs(fishingRemotesList) do
        totalRemotes = totalRemotes + 1
    end
    print("[Fishing] Total remotes found: " .. totalRemotes)
...
-- Test semua remotes yang ditemukan
local function TestAllRemotes()
    print("\n[REMOTE TEST] =========================================")
    print("[REMOTE TEST] Testing all found remotes...")
    
    local tested = 0
    local successful = 0
    
    for name, remote in pairs(fishingRemotesList) do
        tested = tested + 1
        ...
        if success then
            ...
            print("[REMOTE TEST] ✓ " .. name .. " - SUCCESS")
        else
            print("[REMOTE TEST] ✗ " .. name .. " - FAILED")
        end
    ...
    print("[REMOTE TEST] =========================================")
    print("[REMOTE TEST] Results: " .. successful .. "/" .. tested .. " remotes successful")
    print("[REMOTE TEST] =========================================\n")
    
    return successful
end

-- Enhanced fishing function menggunakan semua remotes yang ditemukan
local function EnhancedInstantFishing()
    if not fishingActive then return end
    
    fishingStats.attempts = fishingStats.attempts + 1
    
    -- PHASE 1: INITIATE FISHING (MULTIPLE METHODS)
    local phase1Success = false
    
    -- Method 1: Via WaitingForBite remote
    ...
    -- Method 2: Via Virtual Input (mouse click)
    ...
    if phase1Success then
        task.wait(0.15) -- Tunggu boober di air
        
        -- PHASE 2: SKIP MINIGAME
        if fishingConfig.skipMinigame then
            -- Method 1: Via MinigameStarted remote
            ...
            -- Method 2: Try other potential minigame remotes
            for name, remote in pairs(fishingRemotesList) do
                if name:lower():find("mini") or name:lower():find("game") then
                    pcall(function()
        ...
        -- PHASE 3: CATCH FISH
        local phase3Success = false
        
        -- Method 1: Via ObtainedNewFishNotification
        ...
        -- Method 2: Try other catch-related remotes
        if not phase3Success then
            for name, remote in pairs(fishingRemotesList) do
                if name:lower():find("catch") or name:lower():find("fish") or name:lower():find("obtain") then
                    pcall(function()
                        remote:FireServer()
                        phase3Success = true
                        break
        ...
        -- Method 3: Via Virtual Input (F key)
        if not phase3Success then
            pcall(function()
        ...
        if phase3Success then
            ...
            -- Try other completion remotes
            for name, remote in pairs(fishingRemotesList) do
                if name:lower():find("complete") or name:lower():find("finish") or name:lower():find("done") then
                    pcall(function()
                        remote:FireServer()
                    end)
                end
...
-- Ultra Instant Fishing Mode
local function UltraInstantFishing()
    if not fishingActive then return end
    
    -- Skip semua delay, langsung fire semua remotes
    for name, remote in pairs(fishingRemotesList) do
        pcall(function()
            remote:FireServer()
            remote:FireServer(true)
    ...
    -- Fire semua remotes yang sudah diketahui
    pcall(function()
        if fishingRemotes.WaitingForBite then fishingRemotes.WaitingForBite:FireServer() end
        if fishingRemotes.MinigameStarted then 
            fishingRemotes.MinigameStarted:FireServer("Skip")
            fishingRemotes.MinigameStarted:FireServer(true)
        end
        ...
        if fishingRemotes.CompleteFishing then fishingRemotes.CompleteFishing:FireServer() end
    end)
    
    fishingStats.attempts = fishingStats.attempts + 1
    task.wait(0.0001) -- Hampir no delay
end

-- Main fishing loop function
local function AutoFishingCycle()
    pcall(EnhancedInstantFishing)
end
...
local function StartFishing()
    ...
    -- Pilih fishing function berdasarkan mode
    local fishingFunction = fishingConfig.blantantMode and UltraInstantFishing or AutoFishingCycle
    
    -- Main fishing loop
    fishingConnection = RunService.Heartbeat:Connect(function()
        if fishingActive then
            pcall(fishingFunction)
        end
    end)
...
local function UpdateDetailedRemoteStatus()
    local foundCount = 0
    local details = {}
    ...
    -- Tambahkan remotes dari fishingRemotesList
    local additionalCount = 0
    local additionalDetails = {}
    for name, remote in pairs(fishingRemotesList) do
        if not fishingRemotes[name] then -- Hanya yang belum ada di list utama
            additionalCount = additionalCount + 1
            table.insert(additionalDetails, "• " .. name)
        end
    end
    
    if foundCount == 4 then
        remoteStatus.Text = "✅ ALL MAIN REMOTES FOUND!\n\n" .. 
                          table.concat(details, "\n") .. 
                          "\n\n🔍 Additional Remotes: " .. additionalCount
        remoteStatus.TextColor3 = Color3.fromRGB(100, 255, 100)
    elseif foundCount > 0 then
        remoteStatus.Text = "⚠️ " .. foundCount .. "/4 MAIN REMOTES\n\n" .. 
                          table.concat(details, "\n") .. 
                          "\n\n🔍 Additional Remotes: " .. additionalCount
        remoteStatus.TextColor3 = Color3.fromRGB(255, 200, 100)
    else
        remoteStatus.Text = "❌ NO MAIN REMOTES\n" ..
                          "🔍 Found " .. additionalCount .. " other remotes"
        remoteStatus.TextColor3 = Color3.fromRGB(255, 100, 100)
    end
end

-- ===========================================================
-- 5. ADVANCED CONTROLS PANEL
-- ===========================================================

local controlPanel = Instance.new("Frame")
controlPanel.Size = UDim2.new(1, -24, 0, 100)
controlPanel.Position = UDim2.new(0, 12, 0, 0)
controlPanel.BackgroundColor3 = Color3.fromRGB(14,14,16)
controlPanel.BorderSizePixel = 0
controlPanel.LayoutOrder = 5
controlPanel.Parent = fishingScroll
...
controlTitle.Size = UDim2.new(1, -24, 0, 28)
controlTitle.Position = UDim2.new(0,12,0,8)
controlTitle.BackgroundTransparency = 1
controlTitle.Font = Enum.Font.GothamBold
controlTitle.TextSize = 14
controlTitle.Text = "🎮 Advanced Controls"
controlTitle.TextColor3 = Color3.fromRGB(235,235,235)
controlTitle.TextXAlignment = Enum.TextXAlignment.Left
controlTitle.Parent = controlPanel

-- Tombol Test Remotes
local testRemotesBtn = Instance.new("TextButton")
testRemotesBtn.Name = "TestRemotesBtn"
testRemotesBtn.Size = UDim2.new(0.48, -4, 0, 36)
testRemotesBtn.Position = UDim2.new(0, 12, 0, 40)
testRemotesBtn.BackgroundColor3 = Color3.fromRGB(40, 80, 160)
testRemotesBtn.Font = Enum.Font.Gotham
testRemotesBtn.TextSize = 12
testRemotesBtn.Text = "🧪 Test All Remotes"
testRemotesBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
testRemotesBtn.Parent = controlPanel

local testCorner = Instance.new("UICorner")
testCorner.CornerRadius = UDim.new(0,6)
testCorner.Parent = testRemotesBtn

-- Tombol Refresh Remotes
local refreshRemotesBtn = Instance.new("TextButton")
refreshRemotesBtn.Name = "RefreshRemotesBtn"
refreshRemotesBtn.Size = UDim2.new(0.48, -4, 0, 36)
refreshRemotesBtn.Position = UDim2.new(0.52, 4, 0, 40)
refreshRemotesBtn.BackgroundColor3 = Color3.fromRGB(160, 80, 40)
refreshRemotesBtn.Font = Enum.Font.Gotham
refreshRemotesBtn.TextSize = 12
refreshRemotesBtn.Text = "🔄 Refresh Remotes"
refreshRemotesBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
refreshRemotesBtn.Parent = controlPanel

local refreshCorner = Instance.new("UICorner")
refreshCorner.CornerRadius = UDim.new(0,6)
refreshCorner.Parent = refreshRemotesBtn

-- Set Canvas Size
fishingScroll.CanvasSize = UDim2.new(0, 0, 0, 750)
...
-- Tombol functionality
testRemotesBtn.MouseButton1Click:Connect(function()
    local successful = TestAllRemotes()
    
    -- Update UI dengan hasil testing
    remoteStatus.Text = "🧪 REMOTE TEST COMPLETE!\n\n" ..
                      "Successful: " .. successful .. " remotes\n" ..
                      "Total tested: " .. #fishingRemotesList .. " remotes\n\n" ..
                      "Check console for detailed results."
    remoteStatus.TextColor3 = Color3.fromRGB(100, 200, 255)
    
    -- Reset setelah 3 detik
    task.wait(3)
    UpdateDetailedRemoteStatus()
end)
...
refreshRemotesBtn.MouseButton1Click:Connect(function()
    ...
    remoteStatus.Text = "🔄 REMOTES REFRESHED!\n\n" ..
                      "Found " .. #fishingRemotesList .. " total remotes\n" ..
...
testRemotesBtn.MouseEnter:Connect(function()
    TweenService:Create(testRemotesBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(60, 120, 220)}):Play()
end)

testRemotesBtn.MouseLeave:Connect(function()
    TweenService:Create(testRemotesBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(40, 80, 160)}):Play()
end)

refreshRemotesBtn.MouseEnter:Connect(function()
    TweenService:Create(refreshRemotesBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(200, 120, 60)}):Play()
end)

refreshRemotesBtn.MouseLeave:Connect(function()
    TweenService:Create(refreshRemotesBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(160, 80, 40)}):Play()
end)
...
-- Print summary
print("\n" .. string.rep("=", 50))
print("⚡ NEON HUB - INSTANT FISHING ENHANCED V2.0")
print(string.rep("=", 50))
print("✅ Modern Toggle Switches Implemented")
print("✅ Enhanced Remote Detection")
print("✅ Multiple Fishing Methods")
print("✅ Advanced Remote Testing")
print("✅ Keybind: RightShift to toggle fishing")
print(string.rep("=", 50))
print("🎯 Total Remotes Found: " .. #fishingRemotesList)
print("🎯 Main Remotes: " .. 
      (fishingRemotes.WaitingForBite and "✓" or "✗") .. "WaitingForBite " ..
      (fishingRemotes.MinigameStarted and "✓" or "✗") .. "MinigameStarted " ..
      (fishingRemotes.ObtainedNewFishNotification and "✓" or "✗") .. "ObtainedNewFishNotification " ..
      (fishingRemotes.CompleteFishing and "✓" or "✗") .. "CompleteFishing")
print(string.rep("=", 50) .. "\n")

print("⚡ NEON HUB READY! ⚡")
print("🎣 Fishing is " .. (fishingActive and "ACTIVE" or "PAUSED"))
print("Press RightShift to toggle fishing")