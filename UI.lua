...
local fishingConfig = {
    ...
    disableNotifications = true  -- DISABLE NOTIFICATION BARU!
}
...
-- Hook untuk disable notification
...
local function HookNotificationRemote()
    if fishingRemotes.ObtainedNewFishNotification and not notificationHookActive then
        ...
        -- Hook fungsi FireServer
        fishingRemotes.ObtainedNewFishNotification.FireServer = function(self, ...)
            if fishingConfig.disableNotifications then
                print("[Notification] Notification blocked by Disable Notifications feature")
        ...
        print("[Notification] Cannot hook: Remote not found or already hooked")
        return false
    end
end
...
-- Fungsi untuk toggle disable notifications
local function ToggleDisableNotifications(enabled)
    fishingConfig.disableNotifications = enabled
    
    if enabled then
        -- Coba hook jika belum terhook
        if not notificationHookActive then
...
local function UpdatedAutoFishingCycle()
    ...
    if castSuccess then
        ...
        -- OPTION 2: ObtainedNewFishNotification (TAPI DIBLOKIR JIKA DISABLE NOTIFICATION)
        if not catchSuccess and fishingRemotes.ObtainedNewFishNotification and not fishingConfig.disableNotifications then
            pcall(function()
...
local function UpdatedStartFishing()
    ...
    -- Hook notification jika belum terhook
...
local function StopFishing()
    ...
    -- Unhook notification jika terhook
    if notificationHookActive then
        UnhookNotificationRemote()
    end
...
settingsPanel.Size = UDim2.new(1, -24, 0, 370)
...
-- FITUR BARU: DISABLE NOTIFICATIONS
CreateModernToggle("🔇 Disable Notifications", "Block ObtainedNewFishNotification", fishingConfig.disableNotifications, function(v)
...
local function UpdateEnhancedRemoteStatus()
    ...
    -- Update hook status
    if notificationHookActive then
        hookStatus.Text = "Notification Hook: ✅ ACTIVE (Notifications Blocked)"
        hookStatus.TextColor3 = Color3.fromRGB(100, 255, 100)
    else
        hookStatus.Text = "Notification Hook: ❌ NOT ACTIVE"
        hookStatus.TextColor3 = fishingConfig.disableNotifications and Color3.fromRGB(255, 200, 100) or Color3.fromRGB(255, 100, 100)
    end
end

-- Set Canvas Size
fishingScroll.CanvasSize = UDim2.new(0, 0, 0, 720)
...
spawn(function()
    ...
    -- Hook notification jika disable notifications aktif
...
print("→ 🔇 DISABLE NOTIFICATIONS - Block ObtainedNewFishNotification")
print("→ Enhanced Remote Detection")
print("==========================================")
print("⚠️ Notification Hook Status:", notificationHookActive and "✅ ACTIVE" or "❌ INACTIVE")
print("==========================================")