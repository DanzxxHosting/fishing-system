...
local function ToggleUITransparency(value)
    uiTransparency = math.clamp(value, 0, 0.8)
    isTransparent = uiTransparency > 0
    
    if screen and screen:FindFirstChild("NeonDashboardUI") then
        local container = screen.NeonDashboardUI.Container
        if container then
            TweenService:Create(container.Card, TweenInfo.new(0.3), {
                BackgroundTransparency = uiTransparency
            }):Play()
            
            if uiTransparency > 0.3 then
                container.Card.BorderSizePixel = 1
                container.Card.BorderColor3 = ACCENT
            else
                container.Card.BorderSizePixel = 0
            end
        end
    end
end

-- Transparent UI hotkey (Alt + T)
UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.T and UserInputService:IsKeyDown(Enum.KeyCode.LeftAlt) then
        isTransparent = not isTransparent
        ToggleUITransparency(isTransparent and 0.6 or 0)
    end
end)
...
local function ToggleAutoClicker(enabled)
    ...
    if enabled then
        autoClicker.connection = RunService.Heartbeat:Connect(function()
            ...
            if currentTime - autoClicker.lastClick >= (1 / autoClicker.cps) then
                ...
                if target and target:IsA("BasePart") then
                    ...
                    if character and character.PrimaryPart then
                        ...
                        if distance <= autoClicker.range then
                            mouse1click()
...
local autoFarm = {
    ...
    collecting = false,
    farmDistance = 50,
    farmItems = {"Wood", "Rock", "Ore", "Coin", "Fruit"},
    connection = nil
}

local function CollectNearbyItems()
    if not autoFarm.collecting then
        autoFarm.collecting = true
        ...
        if character and character.PrimaryPart then
            ...
            for _, item in pairs(Workspace:GetChildren()) do
                if item:IsA("BasePart") and not item.Anchored then
                    ...
                    for _, farmItem in pairs(autoFarm.farmItems) do
                        if itemName:find(farmItem:lower()) then
                            shouldCollect = true
                            break
                        end
                    end
                    ...
                    if shouldCollect then
                        local distance = (origin - item.Position).Magnitude
                        if distance <= autoFarm.farmDistance then
...
local function ToggleAutoFarm(enabled)
    ...
    if enabled then
        autoFarm.connection = RunService.Heartbeat:Connect(function()
            if tick() % 2 < 0.1 then -- Collect every 2 seconds
                CollectNearbyItems()
            end
        end)
    else
...
local autoEquip = {
    enabled = false,
    tools = {"Sword", "Axe", "Pickaxe", "Rod", "Tool", "Weapon"},
    connection = nil
}
...
local function EquipBestTool()
    ...
    for _, toolName in pairs(autoEquip.tools) do
...
local function ToggleAutoEquip(enabled)
    autoEquip.enabled = enabled
    
    if enabled then
        autoEquip.connection = player.CharacterAdded:Connect(function()
            task.wait(1)
            EquipBestTool()
        end)
        ...
        if autoEquip.connection then
            autoEquip.connection:Disconnect()
            autoEquip.connection = nil
        end
    end
end
...
local locationsFile = "neon_hub_locations.json"
...
local function SaveLocation(name)
    ...
    if character and character.PrimaryPart then
        ...
        -- Simpan ke DataStore atau file (simplified)
        pcall(function()
            local data = HttpService:JSONEncode(savedLocations)
            writefile(locationsFile, data)
        end)
...
-- Load saved locations on start
pcall(function()
    if isfile(locationsFile) then
        local data = readfile(locationsFile)
        savedLocations = HttpService:JSONDecode(data) or {}
    end
end)
...
local function RejoinServer()
    ...
    if not success then
        warn("[Neon Hub] Failed to rejoin server:", result)
    end
end
...
local function CopyServerID()
    ...
    print("[Neon Hub] Server ID copied to clipboard:", serverId)
end
...
local function ServerHop()
    ...
    if success and #servers > 0 then
        local randomServer
        repeat
            randomServer = servers[math.random(#servers)]
        until randomServer.id ~= game.JobId and randomServer.playing < randomServer.maxPlayers
...
sSubtitle.Text = "Enhanced Utility v4.0"
...
-- BUTTON FUNCTION
-- ===========================================================
local function CreateButton(name, callback, parent, yPos)
...
clickerPanel.Size = UDim2.new(1, -20, 0, 150)
clickerPanel.Position = UDim2.new(0, 10, 0, 0)
clickerPanel.BackgroundColor3 = Color3.fromRGB(14,14,16)
clickerPanel.BorderSizePixel = 0
clickerPanel.LayoutOrder = 1
clickerPanel.Parent = combatScroll

local clickerCorner = Instance.new("UICorner")
clickerCorner.CornerRadius = UDim.new(0,6)
clickerCorner.Parent = clickerPanel

local clickerTitle = Instance.new("TextLabel")
clickerTitle.Size = UDim2.new(1, -20, 0, 24)
clickerTitle.Position = UDim2.new(0,10,0,6)
clickerTitle.BackgroundTransparency = 1
clickerTitle.Font = Enum.Font.GothamBold
clickerTitle.TextSize = 13
clickerTitle.Text = "🖱️ Auto Clicker"
clickerTitle.TextColor3 = Color3.fromRGB(235,235,235)
clickerTitle.TextXAlignment = Enum.TextXAlignment.Left
clickerTitle.Parent = clickerPanel

CreateModernToggle("Enable Auto Clicker", "Automatically click on objects", autoClicker.enabled, function(v)
    ToggleAutoClicker(v)
    utilityConfig.autoClickerEnabled = v
end, clickerPanel, 35)

CreateSlider("CPS (Clicks Per Second)", 1, 30, autoClicker.cps, function(value)
    autoClicker.cps = value
...
farmPanel.Size = UDim2.new(1, -20, 0, 150)
farmPanel.Position = UDim2.new(0, 10, 0, 0)
farmPanel.BackgroundColor3 = Color3.fromRGB(14,14,16)
farmPanel.BorderSizePixel = 0
farmPanel.LayoutOrder = 2
farmPanel.Parent = combatScroll
...
farmTitle.Size = UDim2.new(1, -20, 0, 24)
farmTitle.Position = UDim2.new(0,10,0,6)
farmTitle.BackgroundTransparency = 1
farmTitle.Font = Enum.Font.GothamBold
farmTitle.TextSize = 13
farmTitle.Text = "🌾 Auto Farm"
farmTitle.TextColor3 = Color3.fromRGB(235,235,235)
farmTitle.TextXAlignment = Enum.TextXAlignment.Left
farmTitle.Parent = farmPanel

CreateModernToggle("Enable Auto Farm", "Automatically collect nearby items", autoFarm.enabled, function(v)
    ToggleAutoFarm(v)
    utilityConfig.autoFarmEnabled = v
end, farmPanel, 35)

CreateModernToggle("Auto Equip Tools", "Automatically equip best tools", autoEquip.enabled, function(v)
    ToggleAutoEquip(v)
    utilityConfig.autoEquipEnabled = v
end, farmPanel, 80)

combatScroll.CanvasSize = UDim2.new(0, 0, 0, 330)
...
espPanel.Size = UDim2.new(1, -20, 0, 180)
...
espTitle.Text = "👁️ Visual Effects"
...
CreateModernToggle("💡 Fullbright", "Maximum brightness", utilityConfig.fullbrightEnabled, function(v)
    ToggleFullbright(v)
end, espPanel, 80)

CreateModernToggle("🔍 X-Ray", "See through walls", utilityConfig.xrayEnabled, function(v)
    ToggleXray(v)
end, espPanel, 125)
...
visualScroll.CanvasSize = UDim2.new(0, 0, 0, 330)
...
teleportPanel.Size = UDim2.new(1, -20, 0, 200)
...
CreateDropdown("Teleport to Player", "Select player to teleport", playersList, "Select Player", function(playerName)
    TeleportToPlayer(playerName)
end, teleportPanel, 85)

teleportScroll.CanvasSize = UDim2.new(0, 0, 0, 230)
...
miscPanel.Size = UDim2.new(1, -20, 0, 300)
...
CreateSlider("UI Transparency", 0, 80, utilityConfig.uiTransparency * 100, function(value)
    utilityConfig.uiTransparency = value / 100
    ToggleUITransparency(utilityConfig.uiTransparency)
end, miscPanel, 125)

-- Server Utilities
local serverPanel = Instance.new("Frame")
serverPanel.Size = UDim2.new(1, -20, 0, 150)
serverPanel.Position = UDim2.new(0, 10, 0, 0)
serverPanel.BackgroundColor3 = Color3.fromRGB(14,14,16)
serverPanel.BorderSizePixel = 0
serverPanel.LayoutOrder = 2
serverPanel.Parent = miscScroll
...
serverTitle.Size = UDim2.new(1, -20, 0, 24)
serverTitle.Position = UDim2.new(0,10,0,6)
serverTitle.BackgroundTransparency = 1
serverTitle.Font = Enum.Font.GothamBold
serverTitle.TextSize = 13
serverTitle.Text = "🌐 Server Utilities"
serverTitle.TextColor3 = Color3.fromRGB(235,235,235)
serverTitle.TextXAlignment = Enum.TextXAlignment.Left
serverTitle.Parent = serverPanel

CreateButton("🔄 Rejoin Server", RejoinServer, serverPanel, 35)
CreateButton("📋 Copy Server ID", CopyServerID, serverPanel, 75)
CreateButton("🚀 Server Hop", ServerHop, serverPanel, 115)
...
for name, btn in pairs(menuButtons) do
    btn.MouseButton1Click:Connect(function()
        ...
        combatContent.Visible = (name == "Combat")
...
print("==========================================")
print("⚡ NEON HUB v4.0 - ENHANCED EDITION")
print("==========================================")
...
print("→ Auto respawn")
print("==========================================")
print("⚔️ COMBAT FEATURES:")
print("→ Auto Clicker (1-30 CPS)")
print("→ Auto Farm system")
print("→ Auto equip tools")
print("==========================================")
...
print("→ Teleport to players")
print("→ Save/Load locations")
print("==========================================")
print("🔧 MISC FEATURES:")
print("→ Anti-AFK system")
print("→ Hide username")
print("→ UI Transparency")
print("→ Server utilities")
print("==========================================")
print("🎯 HOTKEYS:")
print("→ Alt + T: Toggle UI transparency")
print("→ Ctrl + T: Teleport to mouse")
print("==========================================")
print("📊 STATUS: Enhanced utility hub ready!")
print("==========================================")