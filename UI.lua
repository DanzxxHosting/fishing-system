...
-- NeoxHub_v2.3.lua - Galaxy Edition (REFINED)
...
-- Blatant modules removed as requested
...
local colors = {
    primary = Color3.fromRGB(0, 200, 255),       -- Cyan/Blue (Neox theme)
    secondary = Color3.fromRGB(147, 112, 219),    -- Medium purple
    accent = Color3.fromRGB(0, 150, 255),        -- Light blue
    galaxy1 = Color3.fromRGB(0, 170, 255),       -- Bright blue
    galaxy2 = Color3.fromRGB(0, 100, 200),       -- Dark blue
...
btnInfo.MouseButton1Click:Connect(function() switchPage("Info", "About Neox Hub") end)
...
FreecamModule.SetMainGuiName("NeoxHub_Galaxy")
...
makeToggle(catHideStats, "⚡ Enable Hide Stats", function(on)
    ...
    if on then
        HideStats.Enable()
        Notify.Send("Hide Stats ✨", "Hide Stats aktif! '-Neox-' berkilau di atas nama.", 3)
    else
        HideStats.Disable()
    ...
    if requestFunc then
        ...
        local testPayload = {
            embeds = {{
                ...
                description = "Your Discord webhook is working correctly!\n\nNeox Hub is ready to send fish notifications." .. filterText,
                color = 3447003,
                footer = {
                    text = "Neox Hub Webhook Test • " .. os.date("%m/%d/%Y %H:%M"),
