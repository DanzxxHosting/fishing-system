...
local function UpdatedAutoFishingCycle()
    ...
    if fishingRemotes.FlyFishingEffect then
        pcall(function() fishingRemotes.FlyFishingEffect:FireServer() end)
    else
    ...
    -- INSTANT CATCH
    task.wait(0.05) -- Tunggu sedikit
    if fishingRemotes.FishCaught then
        pcall(function() fishingRemotes.FishCaught:FireServer() end)
    elseif fishingRemotes.ObtainedNewFishNotification then
        pcall(function() fishingRemotes.ObtainedNewFishNotification:FireServer() end)
    end

    -- SKIP MINIGAME
    if fishingConfig.skipMinigame and fishingRemotes.FishingMinigameChanged then
        pcall(function() fishingRemotes.FishingMinigameChanged:FireServer("Complete") end)
    end
